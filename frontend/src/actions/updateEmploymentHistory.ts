...
export default function updateEmploymentHistorySection(state: GlobalState, payload: EmploymentHistory) {
  ...
  function getGapsFromGroupedRecords(groupedRecords: EmploymentRecord[][]): Array<EmploymentGap> {
    ...
    const latestEmploymentEndDate = new Date(Math.max.apply(null, payload.employmentRecords.map(function(record) {
      return new Date(record.endDate ?? '').getTime();
    })));

    const today = new Date();
    const gapInDays = Math.round((today.getTime() - latestEmploymentEndDate.getTime()) / (1000 * 3600 * 24));

    if (gapInDays >= MIN_EMPLOYMENT_GAP_IN_DAYS) {
      const latestEmploymentRecord = payload.employmentRecords.find(record => new Date(record.endDate ?? '').getTime() === latestEmploymentEndDate.getTime());
      const gap = {
        nameA: latestEmploymentRecord?.name ?? '',
        nameB: 'This Application (Today)',
        startDate: latestEmploymentEndDate.toISOString().split('T')[0],
